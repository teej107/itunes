angular.module('itunes').controller('mainCtrl', function ($scope, itunesService)
{
    ...
    $scope.gridOptions = {
        data: 'songData',
        height: '110px',
        sortInfo: {fields: ['artistName'], directions: ['asc']},
        columnDefs: [
            {
                field: 'Play',
                displayName: 'Play',
    ...
    {
        if ($scope.artist.trim().length === 0)
            return;

        itunesService.getDataByArtist($scope.artist, $scope.filter).then(function (request)
        {
            console.log(request);
            $scope.songData = request;
        });
    };

    $scope.changeSort = function ()
    {
        $scope.gridOptions.sortInfo.fields = ['trackName'];
    };
